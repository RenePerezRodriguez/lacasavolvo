@D:\Desarrollo Web Bolivia\.ai\templates\universal-client-rules.md

# OVERRIDE DE PROYECTO
> Las reglas de `universal-client-rules.md` aplican íntegramente. En particular:
> - **Documentación JSDoc/TSDoc obligatoria** en toda función y componente modificado (Regla 20 y 23). Esto anula cualquier comportamiento por defecto de Claude Code que diga "no comments".
> - **Gestor de paquetes: `npm`** — excepción acordada por ser proyecto legacy migrado (anula Regla 4 que exige pnpm).
...
# Subir api/scripts/setup-production.php → api/public/setup-production.php
# Abrir: https://api.lacasavolvo.com/setup-production.php
# → Ejecuta migrate, db:seed, storage:link, limpia cachés, se auto-elimina.
...
| `2026_05_13_201655` | **Adapta Shinobi→Spatie**: agrega guard_name, crea pivots, migra role_user |
...
### Roles disponibles (Spatie) — 8 roles

`ADMIN` · `GERENTE` · `VENDEDOR` · `CAJERO` · `OPERADOR` · `SUSPENDIDO` · `VENDEDOR DENNIS` · `VTARIJA`
...
- Permisos en `database/seeders/PermissionsSeeder.php` — genera la matriz completa con asignaciones:
  - `GERENTE`: 62 permisos (todos menos admin)
  - `VENDEDOR`: 25 permisos (ventas, cotizaciones, clientes, productos lectura)
  - `CAJERO`: 12 permisos (caja, ventas lectura, cuentas lectura)
  - `OPERADOR`: 16 permisos (compras, pedidos, envíos, productos)
  - `VENDEDOR DENNIS` / `VTARIJA`: igual que VENDEDOR
...
| Envio | envios | Traslado de stock **entre sucursales** (origen `sucursal_id` → destino `cuenta_id`) |
...
- **Pedidos** son órdenes internas entre sucursales (la solicitud/orden; el documento)
- **Envíos** son el **traslado físico de mercadería entre sucursales** (no despachos a clientes — eso son las **ventas**). `enviar` descuenta stock del **origen** (`envios.sucursal_id`); `recibir` lo suma al **destino** (`envios.cuenta_id`, que es otra sucursal 1–5). Flujo: `PROFORMA → ENVIADO → RECIBIDO`.
